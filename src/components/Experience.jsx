    ...
    const listExp = exp.map((expItem, id) => 
        <li key={id} className='exp-card'>
            <div className='exp-description'>
                ...
                <p className='top-margin'>{`${expItem.startDate} — ${expItem.endDate}`}</p>
                
                <ul className='top-margin'>{expItem.experiences.map((experience, id) => {return <li key={id}>{experience}</li>})}</ul>
            </div>
        </li>)
